...
export const updateComment = (commentId, comment) => async (dispatch, getState) => {
  try {
    dispatch({ type: TYPES.UPDATE_COMMENT_LOADING });
    const options = attachTokenToHeaders(getState);
    const response = await axios.put(`${API_URL}/comment/${commentId}`, comment, options);
    dispatch({
...
export const deleteComment = (solutionId, commentId) => async (dispatch, getState) => {
  try {
